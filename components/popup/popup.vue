<template>
	<div style="z-index:9999;overflow:hidden;" v-if="status">
		<!-- 蒙版 -->
		<view v-if="mask" class="position-fixed top-0 left-0 right-0 bottom-0 z-index" :style="getMaskColor"  @touchstart="hide"></view>
		<!-- 弹出框内容 -->
		<div ref="popup" class="position-fixed free-animated z-index bottom-animated " :class="getBodyClass" :style="getBodyStyle" > <!--animation: slide-bottom 3s ease 1 0-->
			<slot></slot>
		</div>
	...
	export default {
		...
		methods: {
			show(x = -1, y = -1) {
				...
				this.x = (x > this.maxX) ? this.maxX-uni.upx2px(this.bodyWidth*0.55) : x-uni.upx2px(this.bodyWidth/2)
				this.y = (y > this.maxY) ? this.maxY : y+10
...
<style scoped>
	...
	.bottom-animated{
		animation: slide-bottom 0.1s ease 1 0s;
	}
